...
<div class="max-w-7xl mx-auto pb-10 space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            ...
            <p class="text-xs text-gray-500 mt-1">{{ $client->company_name ?? 'Individual Client' }} • Registered since {{ $client->created_at->format('d M Y') }}</p>
        </div>
    ...
    <div class="grid grid-cols-1 {{ auth()->user()->can('clients.portal_credentials') ? 'lg:grid-cols-3' : '' }} gap-6">
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6 space-y-4 {{ auth()->user()->can('clients.portal_credentials') ? '' : 'lg:col-span-3' }}">
            <h2 class="text-sm font-bold text-gray-900 uppercase tracking-wider border-b pb-3">Client Information</h2>
            
            <div class="space-y-3 text-xs">
                <div>
                    <span class="text-gray-400 block mb-0.5">PIC Name</span>
                    <span class="font-bold text-gray-800 text-sm">{{ $client->name }}</span>
                </div>
                <div>
                    <span class="text-gray-400 block mb-0.5">Company</span>
                ...
                <div>
                    <span class="text-gray-400 block mb-0.5">Address</span>
        ...
        <div class="lg:col-span-2 bg-white rounded-xl border border-gray-200 shadow-sm p-6 space-y-5">
            <div class="flex items-center justify-between border-b pb-3">
                <div class="flex items-center gap-2">
                    ...
                    <h2 class="text-sm font-bold text-gray-900">Client Portal Access (Wiromitra)</h2>
                </div>
                @php
            ...
            <div class="bg-gray-50 rounded-lg p-4 text-xs text-gray-600 space-y-2 border border-gray-100">
                <p>Wiromitra enables clients to track progress of all assigned projects within an executive portal.</p>
                <div class="flex items-center gap-4 text-xs font-medium text-gray-700 pt-1">
                    <span>Portal URL: <a href="http://localhost:4200" target="_blank" class="text-primary font-bold hover:underline">http://localhost:4200 ↗</a></span>
                </div>
            </div>
            ...
            <div class="flex flex-wrap items-center gap-3 pt-2">
                ...
                <form action="{{ route('clients.portal-instant-credentials', $client) }}" method="POST" onsubmit="return confirm('This will generate a new password for the client. Continue?')">
                    @csrf
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
            <div>
                <h3 class="font-bold text-sm text-gray-900">Client Projects ({{ $client->projects->count() }})</h3>
                <p class="text-xs text-gray-500">All projects are automatically summarized in the client's Wiromitra portal.</p>
            </div>
            @can('projects.create')
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50 text-xs font-bold text-gray-500 uppercase tracking-wider">
                    <tr>
                        <th class="px-6 py-3 text-left">Project Code &amp; Title</th>
                ...
                <tbody class="divide-y divide-gray-200 text-xs">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-gray-500">
                            {{ $project->start_date ? $project->start_date->format('d M Y') : '-' }} to {{ $project->end_date ? $project->end_date->format('d M Y') : '-' }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-right font-medium">
                            <a href="{{ route('projects.show', $project) }}" class="text-primary hover:underline font-bold">View Project ↗</a>
                        </td>
                    </tr>
                    @empty
                    <tr>
