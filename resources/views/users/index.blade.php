...
<div class="max-w-6xl mx-auto">
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-black text-gray-800 tracking-tight">User &amp; Role Management (RBAC)</h1>
            <p class="text-gray-500 text-sm mt-0.5">Manage staff accounts, assign multi-role access, and monitor user activity status.</p>
        </div>
        @can('users.create')
        <a href="{{ route('users.create') }}" class="inline-flex items-center px-4 py-2 bg-primary border border-transparent rounded-lg font-bold text-xs text-white uppercase tracking-wider hover:bg-blue-700 active:bg-blue-900 transition-all shadow-md hover:shadow-lg">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50/80">
                    <tr>
                        <th class="px-6 py-4 text-left text-xs font-bold text-gray-400 uppercase tracking-wider">Name &amp; Email</th>
                        <th class="px-6 py-4 text-left text-xs font-bold text-gray-400 uppercase tracking-wider">Roles</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50/80 transition-colors">
                            ...
                            <div class="flex items-center">
                                ...
                                <div class="ml-3.5">
                                    <div class="flex items-center gap-1.5">
                                        ...
                                        @if($user->id === auth()->id())
                                            <span class="text-[10px] font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full border border-blue-100">You</span>
                                        @endif
                                    </div>
                            ...
                            @can('users.edit')
                                @if($user->id !== auth()->id())
                                    <form action="{{ route('users.toggle-status', $user) }}" method="POST" class="inline-block">
                                        @csrf
                                        <button type="submit" title="Click to toggle status" class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold transition {{ $user->is_active ? 'bg-emerald-50 text-emerald-700 hover:bg-emerald-100 border border-emerald-200' : 'bg-rose-50 text-rose-700 hover:bg-rose-100 border border-rose-200' }}">
                        ...
                        <td class="px-6 py-4 text-xs text-gray-500">
                            <div><span class="text-gray-400">Created:</span> {{ $user->created_at->format('d M Y') }}</div>
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-2">
                                @can('users.edit')
                                <a href="{{ route('users.edit', $user) }}" class="p-1.5 text-blue-600 hover:text-blue-800 hover:bg-blue-50 rounded-lg transition" title="Edit User">
                                ...
                                @can('users.delete')
                                @if($user->id !== auth()->id())
                                <form action="{{ route('users.destroy', $user) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete account {{ $user->name }}?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="p-1.5 text-rose-500 hover:text-rose-700 hover:bg-rose-50 rounded-lg transition" title="Delete User">
